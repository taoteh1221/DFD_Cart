...
$category_data = mysql_query("SELECT * FROM category_structure WHERE id = '".$_GET['updating_category']."'");

$show_category_name = mysql_result($category_data, 0, "category_name");
...
parent.document.getElementById("answers_title").innerHTML = '<b style="font-size: <?=$font_5?>px; color: #BC5938;">Update Category</b>';
...
<div align="center" style="padding: 3px;"><b><u>***Updating a Category***</u></b></div>
<?=$alert_status?>
<div style="padding-top: 9px;"></div>

<div align="center"><b>Category Name:</b><br />
<input type="text" size="25" name="create_category_name" value="<?=$show_category_name?>" /></div>
...
<input type="button" value="Update Category" onclick="window.document.category_editing.return_category.value = window.document.category_editing.category.value; scan_product_form(document.category_editing.create_category_name, '', 'update_category', 1);" />
<input type="hidden" name="return_category" value="<?=$_GET['category']?>" />
<input type="hidden" name="update_current_data" value="yes" />
<input type="hidden" value="<?=$_GET['updating_category']?>" name="category_update_id" />
<input type="hidden" name="validation_attempt" value="update" /></div>
</form>
